...
from sim_evals.inference.droid_jointpos_starvla import Client as DroidJointPosClient
...
def main(
    ...
    client = DroidJointPosClient(remote_port=port,ckpt=ckpt, unnorm_key=unnorm_key)


    video_dir = Path(log_dir) #/ datetime.now().strftime("%Y-%m-%d") / datetime.now().strftime("%H-%M-%S")
