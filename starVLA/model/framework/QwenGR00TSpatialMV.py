...
@FRAMEWORK_REGISTRY.register("QwenGR00TSpatialMV")
class Qwen_GR00TSpatialMV(baseframework):
    """
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                if view_num == 1:
                    ...
                    for i in range(view_num):
                        ...
                        output.append(self.image_edit_model(**inputs))
    ...
    def forward_pass_VLM(self, batch_images, instructions):
        ...
        with torch.autocast("cuda", dtype=torch.float32):
            if self.config.framework.fuser.type == 'cross_attention':
                ...
                if getattr(self, 'spatial_model', None) is not None:
                    if self.spatial_type == "vggt":
                        spatial_tokens = aggregated_tokens_list[-1][:,:,ps_idx:,:]
                        spatial_tokens = spatial_tokens.reshape(len(spatial_tokens), -1, self.config.framework.spatial_model.output_dim)
                    else:
                        raise NotImplementedError
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        if type(examples) is not list:
            examples = [examples]
        if len(examples[0]['image']) > 2:
            batch_images = [to_pil_preserve(example["image"][:-2]) for example in examples]  #  [B，[PLT]]
        else:
            batch_images = [to_pil_preserve(example["image"]) for example in examples]  #  [B，[PLT]]

        primary_images = [images[0] for images in batch_images]
        mv_images = self.forward_pass_image_edit_model(primary_images)
        for i in range(len(batch_images)):
            batch_images[i].append(mv_images[0][i])
            batch_images[i].append(mv_images[1][i])
