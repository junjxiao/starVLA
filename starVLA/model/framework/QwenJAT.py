...
"""
Qwen-JAT Framework
A lightweight implementation that Qwen-VL + JiT-style action head to directly predict continuous actions
JiT-style action head: predicts action samples instead of velocity field (like JiT for images)
"""
import sys
...
@FRAMEWORK_REGISTRY.register("QwenJAT")
class QwenJAT(baseframework):
    """
    Multimodal vision-language-action model.

    Components:
      - Qwen2.5 VL interface for fused language/vision token embeddings
      - JiT-style action head: predicts action samples (not velocity) using DiT architecture
      - Flow matching with sample prediction (similar to JiT for images)

    Focus: Predict future continuous actions conditioned on images + instruction.
    Key difference from QwenGR00T: Uses JiT-style sample prediction instead of direct velocity prediction.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # # ========== 瓶颈结构：低秩线性重参数化 ==========
        # # 从配置中获取瓶颈维度，如果没有则使用默认值
        
        # hidden_size = self.qwen_vl_interface.model.config.hidden_size
        # bottleneck_dim = self.config.framework.action_model.get("bottleneck_dim", 256)
        
        # # 使用nn.Sequential实现瓶颈结构：先降维到瓶颈维度，再恢复到原始维度
        # # 这强制模型学习数据内在的低维流形结构
        # self.hidden_bottleneck = nn.Sequential(
        #     nn.Linear(hidden_size, bottleneck_dim, bias=False),
        #     nn.Linear(bottleneck_dim, hidden_size, bias=True)
        # )
        
        # # 初始化瓶颈层权重
        # # 使用xavier_uniform初始化降维层
        # nn.init.xavier_uniform_(self.hidden_bottleneck[0].weight)
        # # 使用xavier_uniform初始化升维层
        # nn.init.xavier_uniform_(self.hidden_bottleneck[1].weight)
        # # 初始化升维层的偏置为0
        # nn.init.constant_(self.hidden_bottleneck[1].bias, 0)
    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            # total_layers = len(qwenvl_outputs.hidden_states)
            # pruning_ratio = 0.3
            # target_layer_index = round(total_layers * (1 - pruning_ratio))
            # last_hidden = qwenvl_outputs.hidden_states[target_layer_index]

            # last_hidden = self.hidden_bottleneck(last_hidden)
