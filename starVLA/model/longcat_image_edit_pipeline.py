...
class LongCatImageEditPipeline(DiffusionPipeline, FromSingleFileMixin):
    ...
    def __init__(
        ...
        self.image_token = "<tool_call>"
        # NOTE: Removed hard-coded prefix; will build dynamically
        self.system_prompt = "As an image editing expert, first analyze the content and attributes of the input image(s). Then, based on the user's editing instructions, clearly and precisely determine how to modify the given image(s), ensuring that only the specified parts are altered and all other aspects remain consistent with the original(s)."
        self.default_sample_size = 128
        self.tokenizer_max_length = 512

    def _encode_prompt(self, prompt, images):
        """
        Encode prompt with support for multiple images.
        
        Args:
            prompt (List[str]): List of prompts (length 1)
            images (List[PIL.Image.Image]): List of input images
        """
        # Step 1: Process all images with VL processor
        raw_vl_input = self.image_processor_vl(images=images, return_tensors="pt")
        pixel_values = raw_vl_input["pixel_values"]  # [B, C, H, W]
        image_grid_thw = raw_vl_input["image_grid_thw"]  # [B, 3]

        # Step 2: Tokenize user prompt (without image tokens)
        clean_prompt = prompt[0]
        all_tokens = []
        for clean_prompt_sub, matched in split_quotation(clean_prompt):
            if matched:
        ...
        if len(all_tokens) > self.tokenizer_max_length:
            logger.warning(
                f"Prompt truncated! Max length: {self.tokenizer_max_length}, got: {len(all_tokens)}"
            )
        ...
        # Step 3: Build dynamic prompt with correct number of image placeholders
        num_images = len(images)
        # Each image uses 3 tokens in Qwen-VL, so we need 3 * num_images placeholders
        image_placeholders = self.image_token * (3 * num_images)
        full_prompt = (
            f"<|im_start|>system\n{self.system_prompt}<|im_end|>\n"
            f"<|im_start|>user\n{image_placeholders}{clean_prompt}<|im_end|>\n"
            f"<|im_start|>assistant\n"
        )

        # Step 4: Tokenize full prompt
        full_tokens = self.tokenizer(full_prompt, add_special_tokens=False, return_tensors="pt")
        input_ids = full_tokens["input_ids"].to(self.device)
        attention_mask = full_tokens["attention_mask"].to(self.device)

        # Step 5: Move image tensors to device
        pixel_values = pixel_values.to(self.device)
        image_grid_thw = image_grid_thw.to(self.device)

        # Step 6: Forward through text encoder
        ...
        prompt_embeds = text_output.hidden_states[-1].detach()  # [1, seq_len, hidden_size]

        # Step 7: Extract only the user prompt part (excluding system + image tokens)
        # Find the start of user prompt: after all image tokens and before <|im_end|>
        # We know: [system][user][image_tokens][user_prompt][im_end][assistant]
        # For simplicity, we assume the user prompt starts right after the last image token
        vision_start_token_id = self.tokenizer.convert_tokens_to_ids("<tool_call>")
        vision_token_indices = (input_ids[0] == vision_start_token_id).nonzero(as_tuple=True)[0]
        if len(vision_token_indices) > 0:
            prompt_start = vision_token_indices[-1] + 1  # after last image token
        else:
            prompt_start = 0

        # Find end of user prompt (before <|im_end|>)
        im_end_id = self.tokenizer.convert_tokens_to_ids("<|im_end|>")
        im_end_indices = (input_ids[0] == im_end_id).nonzero(as_tuple=True)[0]
        # The first <|im_end|> after user prompt is the one we want
        prompt_end = im_end_indices[1] if len(im_end_indices) > 1 else input_ids.shape[1]

        prompt_embeds = prompt_embeds[:, prompt_start:prompt_end, :]
    ...
    def encode_prompt(
        ...
        # Normalize image to list
        if isinstance(image, PIL.Image.Image):
            image = [image]
        elif not isinstance(image, list):
            raise ValueError("image must be a PIL.Image.Image or a list of PIL.Image.Image")
    ...
    def check_inputs(
        self, prompt, height, width, image=None, negative_prompt=None, prompt_embeds=None, negative_prompt_embeds=None
    ):
            ...
            if isinstance(image, list):
                ...
                for img in image:
                    if not isinstance(img, PIL.Image.Image):
                        raise ValueError("All elements in image list must be PIL.Image.Image")
    ...
    def __call__(
        ...
        # NEW: Handle image list and compute target size
        if isinstance(image, list):
            # Use the first image to determine aspect ratio
            image_size = image[0].size
        else:
            image_size = image.size if image is not None else (512, 512)
        ...
        if image is not None:
            if isinstance(image, list):
                ...
                resized_images = [
                    self.image_processor.resize(img, calculated_height, calculated_width) for img in image
                ]
                processed_image = torch.cat([
                    self.image_processor.preprocess(img, calculated_height, calculated_width) for img in resized_images
                ], dim=0)
                # For prompt encoding, use half-size images
                prompt_image = [
                    self.image_processor.resize(img, calculated_height // 2, calculated_width // 2) for img in image
                ]
            else:
                resized_image = self.image_processor.resize(image, calculated_height, calculated_width)
                processed_image = self.image_processor.preprocess(resized_image, calculated_height, calculated_width)
