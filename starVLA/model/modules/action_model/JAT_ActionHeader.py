...
class FlowmatchingActionHead(nn.Module):
    """
    JiT-style Flow Matching Action Head.
    
    Key difference from standard Flow Matching:
    - Standard Flow Matching: model directly predicts velocity = actions - noise
    - JiT-style: model predicts action samples, then computes velocity = (pred_actions - noisy_trajectory) / (1 - t)
    
    This follows the JiT (Just image Transformer) approach for images, adapted for action prediction.
    """
    def __init__(
        self,
        full_config,
        ...
        # JiT-style sample prediction: add t_eps for numerical stability
        self.t_eps = getattr(config, 't_eps', 5e-2)  # Default from JiT
        self.config = config
    ...
    def forward(self, vl_embs: torch.Tensor, actions: torch.Tensor, state: torch.Tensor = None, encoder_attention_mask=None, action_mask: torch.Tensor = None):
        """
        JiT-style sample prediction forward pass.
        
        Key difference from Flow Matching:
        - Flow Matching: model directly predicts velocity = actions - noise
        - JiT: model predicts sample (actions), then compute velocity = (actions - noisy_trajectory) / (1 - t)
        ...
        # ========== Step 2: Create noisy trajectory (JiT-style linear interpolation) ==========
        ...
        # ========== Step 3: Compute true velocity field (JiT-style) ==========
        ...
        # ========== Step 10: Compute predicted velocity from samples (JiT-style) ==========
    ...
    def predict_action(self, vl_embs: torch.Tensor, state: torch.Tensor = None) -> torch.Tensor:
        """
        JiT-style sample prediction inference.
        
        Key difference from Flow Matching:
        - Flow Matching: model directly predicts velocity, then actions = actions + dt * velocity
        - JiT: model predicts samples, compute velocity from samples, then integrate
        ...
        for t in range(num_steps):
            ...
            # ========== Step 3g: Compute velocity from predicted samples (JiT-style) ==========
...
def get_action_model(config=None):
    """
    Factory: build JiT-style FlowmatchingActionHead from global framework config.
    
    Args:
        config: Global config (expects config.framework.action_model namespace).

    Returns:
        FlowmatchingActionHead: Initialized JiT-style FlowMatchingActionHead.
    """
